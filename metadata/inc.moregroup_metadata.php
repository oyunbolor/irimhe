...
if($metadata_id==1) 
{
	...
	$valueQuery = "count(*) as meta_count, tg.group_name group_name_mn, tg.group_name_en";
	$fromQuery = "FROM ".$schemas.".".$table_name." tm, ".$schemas.".tagroups tg, ".$schemas.".tausergroups tug";
	$groupQuery = "GROUP BY tg.group_name, tg.group_name_en";
	$sortQuery = "ORDER BY tg.group_name";
...
<div class="table-responsive">
  <table class="table table-bordered table-striped table-hover">
	...
	for ($i=$startid; $i < $endid; $i++) 
	{
		...
		$valueQuery = "count(*) as meta_count, tg.group_id, tg.group_name group_name_mn, tg.group_name_en";
		$fromQuery = "FROM ".$schemas.".".$table_name." tm, ".$schemas.".tagroups tg, ".$schemas.".tausergroups tug, ".$schemas.".".$table_name."_h tm_h";
		$groupQuery = "GROUP BY tg.group_id, tg.group_name, tg.group_name_en";
		$sortQuery = "ORDER BY tg.group_name";
