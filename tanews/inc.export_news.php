          ...
          <form class="form" role="form" action="<?php echo $my_url; ?>" method="post" name="mainform" id="mainform">
            <div class="form-row">
              <div class="form-group row col-md-8">
                <label class="col-md-3 col-form-label"><?php echo _p("SearchNewsColumn1"); ?>:</label>
                <div class="col-md-5">
                  <?php
					$selQuery = "SELECT * FROM tamenu_main ORDER BY menu_name ASC";
					$rows = $db->query($selQuery);
					echo seldatadb("export_menu_id", "form-control", $rows, "menu_id", "menu_name", "", "", "Бүх төрөл");
					?>
                </div>
              </div>
			   <div class="form-group row col-md-8">
                <label class="col-md-3 col-form-label"><?php echo _p("SearchNewsColumn2"); ?>:</label>
                <div class="col-md-5">
                  <?php
					$selQuery = "SELECT * FROM tamenu_sub ORDER BY sub_name ASC";
					$row = $db->query($selQuery);
					echo seldatadb("export_sub_id", "form-control", $row, "sub_id", "sub_name", "", "", "Бүх төрөл");
					?>
                </div>
              </div>
            </div>	
