...
<div class="table-responsive">
  <form action="<?php echo $my_url; ?>" method="post" name="mainform" id="mainform" role="form">
    <table class="table table-bordered table-hover">
      ...
      <tbody>
        <?php
			$selQuery = "SELECT taug.group_id, taug.user_id, tag.group_name group_name_mn, tag.group_name_en FROM ".$schemas.".tausergroups taug, ".$schemas.".tagroups tag, ".$schemas.".tausers tau WHERE taug.group_id = tag.group_id AND taug.user_id = tau.user_id AND tau.user_id = ".$user_id." ORDER BY tag.group_name"; 		
        ...
        <tr>
          <td><?php
				...
				?></td>
          <td><button type="submit" class="btn btn-success mb-2" name="insertusergroupbttn"><i class="fa fa-save"></i> <?php echo _p("SaveButton");?></button>
            <button class="btn btn-warning mb-2"><i class="fa fa-undo"></i> <?php echo _p("UndoButton");?></button></td>
        </tr>
        <?php
			}
			?>
        <tr>
          <td colspan="2"><a class="btn btn-success" href="<?php echo $my_url."&actiongroup=groupadd"; ?>"><i class="fa fa-plus"></i> <?php echo _p("AddButton");?></a> <a class="btn btn-secondary" href="<?php echo $my_url_old; ?>"><i class="fa fa-undo"></i> <?php echo _p("BackButton");?></a></td>
        </tr>
      </tbody>
    </table>
  </form>
