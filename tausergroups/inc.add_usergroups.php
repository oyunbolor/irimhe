      ...
      <tbody>
        <tr>
          <td><?php
				...
				?></td>
          <td><?php
				$selQuery = "SELECT tau.user_id, tau.lastname as lastname FROM ".$schemas.".tausers tau ORDER BY tau.organization, tau.lastname";
				$row = $db->query($selQuery);
				if(!empty($row))
					echo seldatadb("user_id", "form-control", $row, "user_id", "lastname", $row[0]["user_id"]);
				else
					echo seldatadb("user_id", "form-control", $row, "user_id", "lastname", NULL);	 	
				?></td>
          <td><button type="submit" class="btn btn-success mb-2" name="insertuserbttn"><i class="fa fa-save"></i> <?php echo _p("SaveButton");?></button>
            <a class="btn btn-warning mb-2" href="<?php echo $my_url.$my_page.$search_url.$sort_url; ?>"><i class="fa fa-undo"></i> <?php echo _p("UndoButton");?></a></td>
        </tr>
      </tbody>
    </table>
  </form>
